...
The below example is the same as the previous, except we're also grouping by year, firstby creating the `Year` variable using `mutate()` and the `lubridate::year()` function.  The result is a summary of the average value by station, parameter, and year.
