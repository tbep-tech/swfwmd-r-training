...
The data we downloaded for this training included water quality measurements and the locations of each station.  If we want to plot any of the data by location, we need to join the two datasets. 
...
1. Select the `Date`, `Station`, and `Value` columns from the training dataset.

1. Filter the dataset by `Station == 'A'`. Check the dimensions of the new dataset with `dim`. 

1. Use a `full_join` to join the dataset with the station location dataset.  What is the key value for joining?  
...
Let's take a look at the training data. Are these data "tidy"? To help with the next few examples, we'll use `pivot_longer()` to manipulate the data.

1. Inspect the training dataset.  What are the dimensions (hint: `dim()`)?  What are the names and column types (hint: `str()`)?

1. If your data have multiple parameter columns, use the `pivot_longer()` function to move the parameter columns and their values into two new columns. What value will you use for the `names_to` argument? What value will you use for the `values_to` argument?  Assign the new dataset to a variable in your environment.
...
We can use the `summarize()` function to get the total or average value by group.  Note the use of the `.by` argument as a critical piece that defines the groups for summarizing.
...
We can group the dataset by more than one column to get summaries with multiple groups.
...
1. Using the long data we created in exercise 7, filter the data by a specific station and parameter.
