...
As before, we have a limited amount of time to cover the basics of data wrangling. It's an unrealistic expectation that you will be a ninja wrangler after this training. As such, the goals are to expose you to fundamentals and to develop an appreciation of what's possible. We also provide resources that you can use for follow-up learning on your own.
...
Combining data is a common task of data wrangling.  Perhaps we want to combine information between two datasets that share a common identifier.  As a real world example, our training data contain measurements from various stations, but we also want to include spatial information about those stations (i.e., latitude, longitude).  We would need to combine data if this information is in two different places.  Combining data with dplyr is called joining.
...
If all keys are shared between two data objects, then inner, left, right, and full joins will give you the same result.
...
The below example is the same as the previous, except we're also grouping by year, first by creating the `Year` variable using `mutate()` and the `lubridate::year()` function (note: `::` lets you use a function without loading the entire package).  The result is a summary of the average value by station, parameter, and year.
...
We can also get more than one summary at a time.  The summary function can use any function that operates on a vector.  Some common examples include `min()`, `max()`, `sd()`, `var()`, `median()`, `mean()`, and `n()`.  It's usually good practice to include a summary of how many observations were in each group using `n()`.
