...
Since the water quality data are collected on specific dates, we can also use `mutate` to convert the date column to different formats. The `lubridate` package allows us to easily extract different time components.  First, we want to make sure the timestamp column is in the correct format.
...
We'll need to convert the time zone from UTC to Eastern Time since `readr::read_csv()` assumed UTC.  We'll use `mutate` and `lubridate::force_tz()` to convert the timestamp column to Eastern Time.  This function is distinct from `lubridate::with_tz()` which will actually change the hour value, i.e., 5pm UTC would be changed to 12pm Eastern Time.  In this example, we want to keep the hour value the same because `readr::read_csv()` assumed the timestamp was in UTC, but it was actually collected in Eastern Time.  The `force_tz()` function will keep the hour value the same, but change the time zone attribute to Eastern Time.  We can verify the actual hour values are in Eastern Time by looking at a histogram of the hours.

```{r}
hist(hour(wqdat$timestamp))
```

Below, we change the time zone attribute with `force_tz`.  Note that the `tzone` argument is specified as `"Etc/GMT+5"`, which is Eastern Time without daylight savings.
...
1. Convert the `timestamp` column to the correct time zone (hint, use `mutate` with `force_tz` for timezone `"Etc/GMT+5"`). 
...
1. Correct the timezone for `timestamp` to `"Etc/GMT+5"` using `mutate` and `force_tz`.
