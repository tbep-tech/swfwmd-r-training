...
* [`dplyr` website](https://dplyr.tidyverse.org/){target="_blank"}
...
Below, we change the time zone attribute with `force_tz`.  Note that the `tzone` argument is specified as `"Etc/GMT+5"`, which is Eastern Time without daylight savings (see `OlsonNames()` for a list of acceptable time zones in R).
...
1. Import the water quality data file from your `data` folder using `read_csv()` (load the tidyverse and here packages first if needed). 
