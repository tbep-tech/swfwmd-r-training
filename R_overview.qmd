...
The data we retrieved included `r nrow(wqdat)` rows and `r ncol(wqdat)` columns. Also note that the data not include all stations.  Only those with water quality data were returned. It may not be immediately obvious that your request did not return all stations until you actually look at the results. 
