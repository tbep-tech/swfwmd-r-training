...
Our request returned information for `r nrow(stationmeta)` active stations (as rows) in Lake Panasoffkee.  There's also a bunch of columns (`r ncol(stationmeta)` to be exact) that we probably don't need, but we'll deal with those later.  For now, we'll use the station numbers (`station_no`) to retrieve water quality data and lake levels using another request.

```{r}
stations <- stationmeta$station_no
...
The data we retrieved did not include all stations.  Only those with water quality data were returned.  Just keep that in mind when retrieving data.  It may not be immediately obviuous that your request did not return data until you actually look at the results. 

```{r}
setdiff(stations, unique(wqdat$station_no))
```

The last data request is for water level data.  These differ from the water quality data because they're "continuous" in time, i.e., time series data, as opposed to "discrete" water quality samples taken at one point in time. We have to request the water level data one station at a time given the amount of data that are retrieved.  The requests also use a `ts_path`: something/station number/something else.  I don't know what this... I hope you do.
...
None of the data we requested is ready for analysis.  Not only do we need to define what we want to do, but we also need to clean it up (wrangle) based on the needs of the analysis.  We'll deal with that later.  For now, we can save the results to retrieve them later. 
...
1. Import `metadat.csv` and `wqdat.csv` using relative file paths.
