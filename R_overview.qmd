...
In most cases, you should write code in scripts and send it to the console as needed. A saved script within a project is your cookbook of instructions to reuse and is not lost when the session closes.
...
An RStudio project also contains a file with the extension `.Rproj`.  This file contains information about the project and is used by RStudio to open the project.  You should not edit it directly.  Instead, use the RStudio interface to manage your project.
...
1. Create a new script in the Source pane and save it as `R/workshop.R`.

1. Add a short header comment (with `#` preceding the text) to the file that explains what the script is for.

1. Download the training zip file [here](data/data.zip), extract it, and copy the provided data into your project `data` folder.

## Accessing SWFWMD data

One goal for this workshop is to use examples that look more like real SWFWMD work. A useful way to do that is to access data from the SWFWMD [Environmental Data Portal](https://www.swfwmd.state.fl.us/resources/data-maps/environmental-data-portal){target="_blank"} and then use those data in later wrangling, plotting, and analysis examples.
...
* and save the result to reuse in later steps of the workflow.
...
* I noticed that the above request for the metadata returned `r ncol(metadat)` columns, but I only requested five fields. This is clearly not working correctly. 
* APIs often change over time, so you may need to consult the online documentation for the correct format of the request (e.g.,URL) if something is not working as expected.
...
1. Import `metadat.csv` and `wqdat.csv` from the data folder with `read_csv()`using relative file paths constructed with the `here()` function.

1. Use `names()`, `dim()`, `head()`, or `glimpse()` to inspect each dataset.

1. Write two or three short notes in your script about what each file appears to contain and what needs to be done to make it ready for analysis.
...
* Do the packages I need load correctly?
...
In this lesson we framed the workshop around a reproducible workflow and imported data we'll use later. The most important themes were to use projects, keep paths relative, inspect data early, read scripts in stages, and troubleshoot carefully. Next, we can build on that foundation with data wrangling tasks that move from clear examples into SWFWMD-style workflows.
