...
With ggplot2, you begin a plot with the function `ggplot()`. `ggplot()` creates a coordinate system for adding "geometries" (aka `geoms`). The first argument of `ggplot()` is the dataset to use in the graph. So, `ggplot(data = toplo)` creates an empty base graph.
