...
Let's make a simple graphic of our training dataset using base R.  We'll make a time series plot of catch using the `plot` function.  We'll also add a title and better axis labels to make the plot slightly more palatable.  

1. First, start by loading the fish data as you've done before.  Use `read_csv()` from the tidyverse. 

1. Make a time series plot of `Value` catch over time using the `plot` function.  Try creating the plot using the `formula` notation (hint: `Value ~ Date`).  Remember you have to use the `data` argument when you're using the formula (i.e., `data = dat`). 

1. Give the plot an informative title using the `main` argument directly in the plot function, something like `main = "Value catch for all stations over time"`.
...
Let's reproduce the time series plot we created in exercise 9 using ggplot. This requires us to map the `Date` and `Value` variables to the x and y aesthetics for `geom_point`. 

1. Setup your initial plot with `ggplot()`.  Map the x variable to `Date` and the y variable to `Value`.  The setup should look like this: `ggplot(dat, aes(x = Date, y = Value))`).  What happens if you run only this code?
...
1. Setup the initial plot as before. Map the x variable to `Date` and the y variable to `Value`.  The initial `ggplot()` line will look like this `ggplot(dat, aes(x = Date, y = Value)`.  Add the `geom_point` geom and the `scale_y_continuous` scale with `trans = "log10"` to transform catch.
