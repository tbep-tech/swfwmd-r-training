...
With ggplot2, you begin a plot with the function `ggplot()` that creates a coordinate system for adding "geometries" (aka `geoms`). The first argument of `ggplot()` is the dataset to use in the graph. So, `ggplot(data = toplo)` creates an empty base graph.
...
The next step is to add one or more `geoms` to `ggplot()`. The function `geom_point()` adds a layer of points to your plot, such as a scatterplot. There are many geom functions that each add a different type of layer to a plot.
...
Each geom function in ggplot2 takes a `mapping` argument. This defines how variables in your dataset are mapped to visual properties called aesthetics. The `mapping` argument is defined with `aes()` for aesthetics, and the `x` and `y` arguments of `aes()` specify which variables to map to the x and y axes. The geom functions look for the mapped variable in the `data` argument, in this case, `toplo`.
...
* Plots require data
* The data are **mapped** to the plot **aesthetics** defined by a **geom** layer
...
Looks like we don't have the required aesthetics, nor does it make sense to use this geom because it's not appropriate for the data.  What about the requirements for `geom_point`?
...
Changing the shapes (e.g., collected during AM or PM times):
...
There are countless ways we can modify a ggplot, either by manipulating the appearance or adding information that improves our understanding of what we see. In this next section we'll cover three additional ways to modify a plot:

* Modifying the appearance of plot attributes can be done using the `theme()` function for individual parts or by using a pre-packaged theme that modifies many parts at once. 
...
Although this is convenient, you don't have fine control over many options that can really make your graphic pop.  I recommend using either the `ggsave` function for ggplot2 graphics, or preferably, one of the available graphics devices from base R (`bmp`, `jpeg`, `png`, `tiff`, `eps`, `ps`, `tex`, `svg`, `wmf`, `pdf`).
